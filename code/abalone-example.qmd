...
format:
  pdf:
    toc: true # use this to display a clickable table of contents
execute:
...
# created new object from abalone
abalone_clean <- abalone |> 
  # cleaned column names (remove spaces and snake case)
  clean_names()  |> 
...
# base layer: ggplot
ggplot(data = abalone_clean, # data frame: abalone clean
       aes(x = mean_ph, # x-axis: pH, y-axis: change in shell surface area/day
           y = change_in_area)) +
  # first layer: points to represent operations
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
**At pH = 8, the model predicts a change in abalone shell surface area to be 5.36 [95% CI: 5.08, 5.64] (mm^2 d^-1^).**
...
ggplot(data = abalone_clean,
       aes(x = mean_ph,
           y = change_in_area)) +
  # first layer: points representing abalones (scatter plot)
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21) +
  # second layer: ribbon representing 95% confidence interval
